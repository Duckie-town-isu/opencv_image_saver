...
def get_pytorch_bbox(img_path):
    ...
    home = str(Path.home()) 
    trained_model = torch.hub.load(home + '/Ranai/Research/yolov5', 'custom', path=home + '/Ranai/Research/yolov5/duckietown_weights/best.pt', source='local', force_reload=True)
    ...
    if len(pred_list) > 0:
        for pred in pred_list:
            ...
            predictions[obj_class].append({"tag":f"{obj_class}{counter[obj_class]}", "class": f"{obj_class}", "bbox": [topleft[0], topleft[1], botright[0], botright[1]]})
